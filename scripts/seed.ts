/**
 * Popula o banco com tudo que já foi levantado sobre o Coffee Car:
 * cardápio/insumos, os 3 eventos fechados até agora (SPIW 2026, Adapta Summit, NBA)
 * e usuários de acesso inicial. Rode com: npm run seed
 * É seguro rodar mais de uma vez (verifica se já existe dado antes de inserir).
 */
...
async function main() {
  if (await alreadySeeded()) {
    console.log("Banco já tem eventos cadastrados — pulando seed para não duplicar dados.");
    return;
  }
  ...
  if ((await repo.countUsers()) === 0) {
    ...
    console.log(`Usuários criados. Senha temporária para todos: "${tempPassword}" (troque depois do primeiro login).`);
  }

  // ---------- Insumos (fonte: PLANILHA DE CUSTOS CAFE.xlsx, 14/08/2026) ----------
  ...
  const byName = (n: string) => accounts.find((a) => a.name === n)?.id ?? null;
  await repo.createLedgerEntry({ date: "2026-07-31", amount: 1377.82, reason: "Compra insumo", eventId: adaptaId, fromAccountId: null, toAccountLabel: byName("Conta Santander") ? "Conta Santander" : "Conta Santander", whoReturns: "C6 - Coffee Car", notes: null });
  ...
  console.log("Seed concluído: 3 eventos, insumos, staff, contas e movimentações criados.");
}
